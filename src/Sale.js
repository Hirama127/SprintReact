...
const Sale = () => {
    ...
    const [loading, setLoading] = useState(true);    //データが読み込み中の状態
    ...
    //データの取得と状態の更新
    useEffect(() => {
        fetch(saleUrl)   //URL (saleUrl) からデータを取得

            //fetchメソッドの非同期処理が成功した後に実行されるコールバック関数を定義
            .then(response => response.json())    //取得したデータをJSON形式に変換
            .then(data => {      //dataという変数に代入
                console.log(saleData);
                setSaleData(data);   //SaleDataのデータを更新
                setLoading(false);   //データの読み込みが完了した状態
            })
            .catch(error => {
                setLoading(false);   //データの読み込みが完了した状態
            });
        ...
        <div>
            {loading ? (    //ロード中の状態（true）
                ...
                <table className="saleTable">
                    ...
                    <tbody>
                        {saleData.map((item, Data) => (    //saleData配列の要素ごとに処理を行うためのメソッド
